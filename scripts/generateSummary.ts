...
/**
 * 从 .env 文件读取指定键，优先 .env 文件，其次 process.env
 */
...
function readCleanBeforeAPIFromCustom(): boolean | null {
  ...
  const m = content.match(
    /^[\t ]*\/\/\s*AISUMMARY_CLEAN_BEFORE_API\s*[:=]\s*(true|false|1|0)\s*$/im,
  );
  if (!m) return null;
...
function readCleanBeforeAPIFromEnv(): boolean | null {
  const raw = getEnv("AISUMMARY_CLEAN_BEFORE_API").trim();
  if (!raw) return null;
...
function readWordLimitFromCustom(): number | null {
  ...
  const content = fs.readFileSync(customPath, "utf-8");
  const m = content.match(
    /^\s*\/\/\s*AISUMMARY_WORD_LIMIT\s*[:=]\s*([0-9]+)\s*$/m,
  );
  if (!m) return null;
...
function readWordLimitFromEnv(): number | null {
  // 优先 AISUMMARY_MAX_TOKEN（本博客 .env 配置），其次 AISUMMARY_WORD_LIMIT
  const envVal =
    getEnv("AISUMMARY_MAX_TOKEN") || getEnv("AISUMMARY_WORD_LIMIT");
...
function readLogLevelFromCustom(): LogLevel | null {
  ...
  const content = fs.readFileSync(customPath, "utf-8");
  const m = content.match(
    /^\s*\/\/\s*AISUMMARY_LOG_LEVEL\s*[:=]\s*([0-2])\s*$/m,
  );
  if (!m) return null;
  const v = parseInt(m[1], 10);
...
function readLogLevelFromEnv(): LogLevel | null {
  const envVal = getEnv("AISUMMARY_LOG_LEVEL");
  const v = parseInt(envVal, 10);
  return v === 0 || v === 1 || v === 2 ? (v as LogLevel) : null;
}
...
function readConcurrencyFromCustom(): number | null {
  ...
  const content = fs.readFileSync(customPath, "utf-8");
  const m = content.match(
    /^\s*\/\/\s*AISUMMARY_CONCURRENCY\s*[:=]\s*([0-9]+)\s*$/m,
  );
  if (!m) return null;
  const v = parseInt(m[1], 10);
  return Number.isFinite(v) && v > 0 ? v : null;
...
function readConcurrencyFromEnv(): number | null {
  const envVal = getEnv("AISUMMARY_CONCURRENCY");
  const v = parseInt(envVal, 10);
  return Number.isFinite(v) && v > 0 ? v : null;
}
...
function readOverwritePolicyFromCustom(): OverwritePolicy | null {
  ...
  const content = fs.readFileSync(customPath, "utf-8");
  const m = content.match(
    /^\s*\/\/\s*AISUMMARY_OVERWRITE_EXISTING\s*[:=]\s*(\w+)\s*$/im,
  );
  if (!m) return null;
  const v = m[1].toLowerCase();
  if (v === "ask" || v === "always" || v === "never")
    return v as OverwritePolicy;
...
function readOverwritePolicyFromEnv(): OverwritePolicy | null {
  // 优先 AISUMMARY_OVERWRITE_EXISTING，其次 AISUMMARY_COVER_ALL（本博客 .env 配置）
  const overwrite = getEnv("AISUMMARY_OVERWRITE_EXISTING");
  const coverAll = getEnv("AISUMMARY_COVER_ALL");
  if (overwrite) {
    const raw = overwrite.trim().toLowerCase();
    if (raw === "ask" || raw === "always" || raw === "never")
      return raw as OverwritePolicy;
    if (["yes", "y", "true", "1"].includes(raw)) return "always";
    if (["no", "n", "false", "0"].includes(raw)) return "never";
  }
  if (coverAll) {
    const v = coverAll.trim().toLowerCase();
    if (["true", "1", "yes", "y"].includes(v)) return "always";
    if (["false", "0", "no", "n"].includes(v)) return "never";
  }
  return null;
}
...
function looksLikeCode(text: string): boolean {
  ...
  if (
    /\b(import|export|const|let|var|function|interface|class|return|new)\b/.test(
      text,
    )
  )
    score += 2;
 ...
 */
function upsertSummaryInFrontmatter(
  frontmatter: string,
  summary: string,
): string {
  if (!frontmatter) {
  ...
  const bodyLines = lines.slice(1, endIdx).filter((l) => {
...
async function callSummaryAPI(
  ...
  const api = cfg.api || getEnv("AI_SUMMARY_API");
  const key = cfg.key || getEnv("AI_SUMMARY_KEY");
  if (!api) return null;
  ...
  try {
    ...
    // 约定返回 { summary: string }
    return typeof (data as any).summary === "string"
      ? (data as any).summary.trim()
      : null;
  } catch (err) {
...
async function run(): Promise<void> {
  ...
  log(
    1,
    `日志等级已设置为 ${currentLogLevel}（${currentLogLevel === 0 ? "错误" : currentLogLevel === 1 ? "信息" : "调试"}）`,
  );
  ...
  const cfg = readAIConfigFromCustom();
  const hasCustomAPI = !!(cfg.api || getEnv("AI_SUMMARY_API"));

  // 调试说明：脚本仅处理 blog 目录的文章，不处理页面文件
  log(2, `跳过页面，仅处理文章目录：${BLOG_DIR}`);
  log(
    1,
    `待处理文章数：${files.length}，字数限制：${wordLimit}，并发：${concurrency}`,
  );
    ...
    try {
      ...
      if (hasSummaryInFrontmatter(frontmatter)) {
        ...
        if (overwritePolicy === "never") {
          log(
            1,
            `跳过已有摘要：${path.relative(ROOT, file)}（预览：${previewText(existing, 80)}）`,
          );
          return;
        }
      ...
      const contentForAPI = cleanBeforeAPI
        ? sanitizeBodyForAPI(limitedBody)
        : limitedBody;
      if (cleanBeforeAPI) {
      ...
      log(
        2,
        `API 仅提交正文（忽略 frontmatter）：${path.relative(ROOT, file)}`,
      );
      // 预览提交给 API 的正文片段，确认不包含 frontmatter
      log(
        2,
        `API 提交文本预览（前 120 字）：${previewText(contentForAPI, 120)}`,
      );
      const apiSummary = hasCustomAPI
        ? await callSummaryAPI(title, contentForAPI, limit)
        : null;
      ...
      if (!summary || looksLikeCode(summary)) {
        log(
          1,
          `API 失败或内容不适合摘要，使用本地规则：${path.relative(ROOT, file)}`,
        );
