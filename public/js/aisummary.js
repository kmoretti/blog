// AISummary v1.0.0 — 适配 Astro View Transitions
console.log(
  "%cAISummary v1.0.0 %c https://github.com/ljxme/static-aisummary",
  "color:#fff;background:linear-gradient(90deg,#3b82f6,#60a5fa);padding:5px 12px;border-radius:4px 0 0 4px;",
  "color:#000;background:linear-gradient(90deg,#60a5fa,#bfdbfe,#fff);padding:5px 14px 5px 8px;border-radius:0 4px 4px 0;",
);
...
const aisummary = {
  ...
  getStaticSummary() {
    ...
    const text =
      attr && attr.trim().length ? attr : (el.textContent || "").trim();
    return text || "";
  },

  /**
   * 展示摘要：根据开关决定是否执行打字机动画
   */
...
/**
 * 多时机初始化，兼容 Astro View Transitions
 */
(function ensureInit() {
  // 标准 DOM 加载
